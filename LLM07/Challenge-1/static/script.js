...
const leakState = document.getElementById("leakState");

function renderLeakState(leaked) {
  if (!leakState) return;
  leakState.textContent = leaked ? "Yes" : "No";
  leakState.classList.toggle("leaked", !!leaked);
}
...
function applyState(data) {
  ...
  renderLeakState(data.instructions_leaked);
  if (data.solved) {
...
async function sendMessage() {
  ...
  const pendingEl = appendPending("translating...");
