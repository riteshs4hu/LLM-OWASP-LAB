let sessionId = localStorage.getItem("llm02c2_session_id") || null;
...
const userIdCustom = document.getElementById("userIdCustom");
...
function ensureSessionId() {
  if (!sessionId) {
    ...
    localStorage.setItem("llm02c2_session_id", sessionId);
  }
  return sessionId;
}

function currentUserId() {
  if (userIdSelect.value === "__custom__") {
    return userIdCustom.value.trim() || "bob";
  }
  return userIdSelect.value;
}

function appendLine(role, text, asUser) {
  hideEmptyState();
  ...
  if (role === "user") {
    if (asUser) {
      ...
      tag.className = "acting-as-tag";
      tag.textContent = `as ${asUser}`;
...
function renderMsgTable(rows) {
  msgCount.textContent = rows.length;
  if (!rows.length) {
    msgTable.innerHTML = `<div class="apply-msg">No messages yet.</div>`;
    return;
  }
  const trs = rows.map(m => {
    const content = m.content.length > 60 ? m.content.slice(0, 57) + "..." : m.content;
    return `<tr><td>${escapeHtml(m.user_id)}</td><td>${escapeHtml(m.role)}</td><td>${escapeHtml(content)}</td></tr>`;
  }).join("");
  msgTable.innerHTML = `<table class="mini-table"><thead><tr><th>User</th><th>Role</th><th>Content</th></tr></thead><tbody>${trs}</tbody></table>`;
}

async function refreshMessages() {
  if (!sessionId) return;
  try {
    const res = await fetchWithTimeout(`/api/messages?session_id=${encodeURIComponent(sessionId)}`, {}, 10000);
    if (!res.ok) return;
    const data = await res.json();
    if (data && Array.isArray(data.messages)) renderMsgTable(data.messages);
  } catch (e) {
  }
}
...
async function rehydrateConversation() {
  ...
  try {
    ...
    const mine = data.messages.filter(m => m.user_id === identity);
    mine.forEach(m => {
      if (m.role === "user") appendLine("user", m.content, identity);
      else if (m.role === "assistant") appendLine("assistant", m.content);
    });
  } catch (e) {
  }
}
...
async function sendMessage() {
  ...
  const actingAs = currentUserId();
  appendLine("user", raw, actingAs);
  ...
  if (sessionId) localStorage.setItem("llm02c2_session_id", sessionId);
...
userIdSelect.addEventListener("change", () => {
  userIdCustom.classList.toggle("hidden", userIdSelect.value !== "__custom__");
});
...
resetBtn.addEventListener("click", async () => {
  ...
  sessionId = null;
  localStorage.removeItem("llm02c2_session_id");
