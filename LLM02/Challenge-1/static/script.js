let sessionId = localStorage.getItem("llm02c1_session_id") || null;
let conversation = JSON.parse(localStorage.getItem("llm02c1_conversation") || "[]");

function saveConversation() {
  localStorage.setItem("llm02c1_conversation", JSON.stringify(conversation));
}
...
function ensureSessionId() {
  if (!sessionId) {
    ...
    localStorage.setItem("llm02c1_session_id", sessionId);
  }
  return sessionId;
}
...
function renderToolEvents(container, events) {
  ...
  events.forEach(ev => {
    const tool = ev && ev.tool ? ev.tool : "(unknown)";
    const detail = ev && ev.detail ? ev.detail : "";
    const insecure = ev && ev.insecure_result ? ev.insecure_result : "(no result)";
    const secure = ev && ev.secure_result ? ev.secure_result : "";
    const violated = !!(ev && ev.violated_policy);

    const box = document.createElement("div");
    box.className = "tool-event" + (violated ? " violated" : "");
    ...
    header.innerHTML = `
      ...
      ${violated ? `<span class="violation-badge">Policy violated</span>` : ""}
    `;
    box.appendChild(header);

    const body = document.createElement("div");
    body.className = "tool-event-body" + (secure ? "" : " single");

    if (secure) {
      body.innerHTML = `
        <div class="result-block insecure">
          <div class="result-label">Insecure backend</div>
          <div class="result-text">${escapeHtml(insecure)}</div>
        </div>
        <div class="result-block secure">
          <div class="result-label">Secure backend</div>
          <div class="result-text">${escapeHtml(secure)}</div>
        </div>
      `;
    } else {
      body.innerHTML = `
        <div class="result-block plain">
...
async function sendMessage() {
  ...
  if (sessionId) localStorage.setItem("llm02c1_session_id", sessionId);

  if (data.reply) conversation.push({ role: "assistant", content: data.reply });
  saveConversation();

  const replyLine = appendLine("assistant", data.reply || "");
...
resetBtn.addEventListener("click", async () => {
  ...
  sessionId = null;
  conversation = [];
  localStorage.removeItem("llm02c1_session_id");
  localStorage.removeItem("llm02c1_conversation");
