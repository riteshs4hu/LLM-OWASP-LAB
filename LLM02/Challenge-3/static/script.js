let sessionId = localStorage.getItem("llm02c3_session_id") || null;
let conversation = JSON.parse(localStorage.getItem("llm02c3_conversation") || "[]");

function saveConversation() {
  localStorage.setItem("llm02c3_conversation", JSON.stringify(conversation));
}
...
function ensureSessionId() {
  if (!sessionId) {
    ...
    localStorage.setItem("llm02c3_session_id", sessionId);
  }
  return sessionId;
}
...
async function refreshStatus() {
  ...
  try {
    ...
    if (data && typeof data === "object") {
      ...
      if (Number.isFinite(data.tool_calls)) callCount.textContent = data.tool_calls;
      if (typeof data.username === "string" && data.username) usernameInput.value = data.username;
...
async function sendMessage() {
  ...
  if (sessionId) localStorage.setItem("llm02c3_session_id", sessionId);
...
resetBtn.addEventListener("click", async () => {
  ...
  sessionId = null;
  conversation = [];
  localStorage.removeItem("llm02c3_session_id");
  localStorage.removeItem("llm02c3_conversation");
