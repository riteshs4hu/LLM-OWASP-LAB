...
const mPeakWindow = document.getElementById("mPeakWindow");
const mInFlight = document.getElementById("mInFlight");
const mPeakConcurrent = document.getElementById("mPeakConcurrent");
const mTokens = document.getElementById("mTokens");
const mRefused = document.getElementById("mRefused");
const meterFill = document.getElementById("meterFill");
const meterCap = document.getElementById("meterCap");
const meterCapVal = document.getElementById("meterCapVal");

let budgetRequests = 30;
...
function applyState(data) {
  if (!data) return;
  if (data.budget_requests) budgetRequests = data.budget_requests;

  setText(attemptCount, fmt(data.attempts));
  setText(totalRequests, fmt(data.total_requests));
  setText(mWindow, fmt(data.requests_in_window));
  setText(mPeakWindow, fmt(data.peak_window_requests));
  setText(mInFlight, fmt(data.in_flight));
  setText(mPeakConcurrent, fmt(data.peak_concurrent));
  setText(mTokens, fmt(data.tokens_billed));
  setText(mRefused, fmt(data.refused_by_rate_limit));

  const inWindow = Number(data.requests_in_window || 0);
  const pct = budgetRequests ? Math.min(100, (inWindow / budgetRequests) * 100) : 0;
  if (meterFill) {
    meterFill.style.width = pct.toFixed(1) + "%";
    meterFill.classList.toggle("warn", pct >= 60 && pct < 100);
    meterFill.classList.toggle("danger", pct >= 100);
  }
  setText(meterCapVal, inWindow + " / " + budgetRequests);
  if (meterCap) meterCap.classList.toggle("over", inWindow > budgetRequests);
...
async function sendMessage() {
  ...
  appendLine("assistant", data.reply || "(the assistant returned an empty reply)");
  scrollToBottom();
  applyState(data);
  userInput.focus();
}
