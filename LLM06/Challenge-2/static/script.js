...
function renderCalendar(events) {
  ...
  if (!Array.isArray(events) || !events.length) {
    ...
    empty.textContent = "Nothing left on the calendar.";
  ...
  events.forEach((ev) => {
    ...
    when.textContent = ((ev.weekday || ev.day || "") + " " + (ev.time || "")).trim();
...
function renderToolEvents(container, events) {
  ...
  events.forEach((ev) => {
    if (!ev) return;
    const destroyed = ev.tool === "delete_event" && ev.deleted;

    const box = document.createElement("div");
    box.className = "tool-event" + (destroyed ? " violated" : "");
    ...
    if (destroyed) {
      header.innerHTML +=
        '<span class="tool-detail">' + escapeHtml(ev.event_id || "") + "</span>" +
        '<span class="violation-badge">deleted, no confirmation</span>';
    } else if (ev.error) {
    ...
    if (destroyed) {
      const when = document.createElement("div");
      when.className = "call-line";
      when.textContent = [ev.weekday, ev.day, ev.time].filter(Boolean).join(" ");

      const title = document.createElement("div");
      title.className = "result-line deleted";
      title.textContent = ev.title || "(untitled)";
      ...
      if (Array.isArray(ev.attendees) && ev.attendees.length) {
        ...
        attendees.className = "result-line";
        attendees.textContent = "attendees: " + ev.attendees.join(", ");
...
async function sendMessage() {
  ...
  if (attemptCount) {
    attemptCount.textContent = Number.isFinite(data.deleted_count) ? data.deleted_count : "?";
  }
  if (remainingCount) {
    remainingCount.textContent = Number.isFinite(data.events_remaining) ? data.events_remaining : "?";
  }
...
async function refreshCalendar() {
  const data = await postJSON("/api/calendar", { session_id: sessionId }, 10000);
  if (!data) return;
  renderCalendar(data.events);
  if (attemptCount) attemptCount.textContent = String(data.deleted_count || 0);
  if (remainingCount) remainingCount.textContent = String(data.events_remaining || 0);
}
