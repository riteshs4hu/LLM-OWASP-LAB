let sessionId = localStorage.getItem("challenge2_session_id") || null;
let conversation = JSON.parse(localStorage.getItem("challenge2_conversation") || "[]");

function saveConversation() {
  localStorage.setItem("challenge2_conversation", JSON.stringify(conversation));
}
...
function renderToolEvents(container, events) {
  ...
  events.forEach(ev => {
    ...
    box.className = "tool-event";
    const tool = ev && ev.tool ? ev.tool : "(unknown)";
    const detail = ev && ev.detail ? ev.detail : "";
    const result = ev && ev.result ? ev.result : "(no result)";
    box.innerHTML = `
      <div class="call-line">model called ${escapeHtml(tool)}(${escapeHtml(detail)})</div>
      <div class="result-line ${ev && ev.violated_policy ? 'deleted' : ''}">${escapeHtml(result)}</div>
    `;
...
function markSolved(reason) {
  ...
  solvedReasonEl.textContent = reason || "transfer_money executed without server-side re-authorization.";
...
async function sendMessage() {
  ...
  if (sessionId) localStorage.setItem("challenge2_session_id", sessionId);

  conversation = outgoingMessages.slice();
  ...
  if (data.tool_events && data.tool_events.length) {
    renderToolEvents(replyLine, data.tool_events);
  }

  attemptCount.textContent = Number.isFinite(data.attempts) ? data.attempts : "?";
  balanceLeft.textContent = typeof data.balance === "number" ? `$${data.balance.toFixed(2)}` : "?";
...
resetBtn.addEventListener("click", async () => {
  ...
  sessionId = null;
  conversation = [];
  localStorage.removeItem("challenge2_session_id");
  localStorage.removeItem("challenge2_conversation");
  ...
  balanceLeft.textContent = "$12000.00";
  updateCharCount();
});
