...
function renderToolEvents(col, events) {
  if (!col || !Array.isArray(events) || !events.length) return;
  events.forEach((ev) => {
    const box = document.createElement("div");
    box.className = "tool-event";
    const id = ev && ev.record_id ? ev.record_id : "(none)";
    const result = ev && ev.result ? ev.result : "(no result)";
    box.innerHTML =
      '<div class="call-line">model called ' + escapeHtml(ev.tool || "tool") +
      '("' + escapeHtml(id) + '")</div>' +
      '<div class="result-line' + (ev && ev.violation ? " deleted" : "") + '">' +
...
function applyState(data) {
  if (!data) return;
  if (attemptCount) attemptCount.textContent = data.attempts || 0;
  if (toolCallCount) toolCallCount.textContent = data.tool_calls || 0;
...
async function sendMessage() {
  ...
  const line = appendLine("assistant", data.reply || "(empty reply)");
...
if (resetBtn) {
  resetBtn.addEventListener("click", async () => {
    ...
    clearThread();
    clearError();
    applyState({ attempts: 0, tool_calls: 0, solved: false, solved_reason: "" });
  });
}
